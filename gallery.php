...
?>
<style>
#portfolio .img-fluid{      /* Styles for portfolio image */
    width: calc(100%);
    height: 30vh;
    z-index: -1;
    position: relative;
...
.gallery-list{      /* Style for gallery card container */
cursor: pointer;
border: unset;
flex-direction: inherit;
}
.gallery-img,.gallery-list .card-body {     /* Splits image and text in gallery card */
    width: calc(50%)
}
.gallery-img img{       /* Style for images inside the gallery */
    border-radius: 5px;
    min-height: 50vh;
    max-width: calc(100%);
}
span.hightlight{        /* Highlights specific text */
    background: yellow;
}
...
.card-left{     /* Optional alignment for cards */
    left:0;
}
.card-right{
    right:0;
}
.rtl{       /* Sets direction to right-to-left */
    direction: rtl ;
}
.gallery-text{      /* Center gallery text vertically and horizontally */
     ...
     .masthead:before{
        ...
        <header class="masthead">
            ...
            <div class="container-fluid mt-3 pt-2">
                ...
                }       // Fetch gallery entries from database
                $gallery = $conn->query("SELECT * from gallery order by id desc");
                while($row = $gallery->fetch_assoc()):
                   
                    $ci++;
                    if($ci < 3){        // Reset direction based on item count
                        $rtl = '';
                    }else{
                        $rtl = 'rtl';
                    }
